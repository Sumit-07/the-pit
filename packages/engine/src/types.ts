...
/**
 * The four anchor levels every rubric metric carries. Source: `01 §4` Step 2 —
 * `validate_jury` requires all four keys `"100" "80" "50" "20"`, each non-empty.
 *
 * Keyed by the score they describe, exactly as the installed jury JSON is, so the
 * file and the type never need translating between each other.
 */
...
/**
 * A rubric metric as the scoring prompt needs it: the `Metric` the board carries
 * plus the four anchors `01 §5.1` requires the juror to be shown.
 * Source: `01 §4` Step 2.
 *
 * Produced by jury generation (Task 6); consumed by `buildScoreRequest`.
 */
export interface RubricMetric extends Metric {
  anchors: MetricAnchors;
}

/**
 * One installed juror mandate. Source: `01 §4` Step 2 (`validate_jury`) — every
 * field non-empty, `role` unique across the jury, `weights` keyed by exactly the
 * rubric's metric names.
 *
 * These five prose fields are INSTRUCTIONS, not data: they are generated by the
 * jury pass and pass an explicit human approval gate before any run. The scoring
 * prompt therefore places them outside the `<<< >>>` data block (Global
 * Constraint 2).
 */
export interface JurorMandate extends JurorWeights {
  who: string;
  cares_most: string;
  biased_against: string;
  voice: string;
}
